...
import com.inphase.okhttputils.OkHttpUtils;
import com.inphase.okhttputils.callback.AbsCallback;
...
public abstract class BaseRequest<R extends BaseRequest> {
		...
		return new ProgressRequestBody(requestBody,
				new ProgressRequestBody.Listener() {
					...
					public void onRequestProgress(final long bytesWritten,
							final long contentLength, final long networkSpeed) {
						OkHttpUtils.getInstance().getDelivery()
								.post(new Runnable() {
									@Override
									public void run() {
										if (mCallback != null)
											mCallback.upProgress(bytesWritten,
	...
	public Call generateCall(Request request) {
		if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0
			...
			OkHttpClient.Builder newClientBuilder = OkHttpUtils.getInstance()
			...
			if (certificates != null)
				newClientBuilder.sslSocketFactory(HttpsUtils
						.getSslSocketFactory(certificates, null, null));
	...
	public <T> void execute(AbsCallback<T> callback) {
		mCallback = callback;
		if (mCallback == null)
			mCallback = AbsCallback.CALLBACK_DEFAULT;
		...
		call.enqueue(new okhttp3.Callback() {
			@Override
			public void onFailure(Call refcall, IOException e) {
				// TODO Auto-generated method stub
				sendFailResultCallback(refcall.request(), null, e, mCallback);
			}

			@Override
			public void onResponse(Call refcall, Response response)
					throws IOException {
				// TODO Auto-generated method stub
				// 响应失败，一般为服务器内部错误，或者找不到页面等
				if (response.code() >= 400 && response.code() <= 599) {
					sendFailResultCallback(response.request(), response, null,
							mCallback);
					return;
				}

				try {
					// 判断是否token失效500或结果错误,根据贝壳商城后台接口定制修改

					// 解析过程中抛出异常，一般为 json 格式错误，或者数据解析异常
					T t = (T) mCallback.parseNetworkResponse(response);
					sendSuccessResultCallback(t, response.request(), response,
							mCallback);
				} catch (CustomException e) {
					if (e.getErrCode() == Consts.CODE_TOKENINVALID_ERROR) {
						sendTokeninvalidCallback(response.request(), response,
								null, mCallback);
					} else {
						sendErrorResultCallback(
								response.request(),
								response,
								new ErroBean(e.getErrCode(), e.toString(), e
										.getMsgflag()), mCallback);
					}
				} catch (Exception e) {
					sendFailResultCallback(response.request(), response, e,
							mCallback);
				}
			}
		});
	...
	/** 失败回调，发送到主线程 */
	public <T> void sendFailResultCallback(final Request request,
	...
	/** 成功回调，失败结果 */
	public <T> void sendErrorResultCallback(final Request request,
			final Response response, final ErroBean msg,
			final AbsCallback<T> callback) {
		OkHttpUtils.getInstance().getDelivery().post(new Runnable() {
			@Override
			public void run() {
				callback.onError(msg); // 请求失败回调 （UI线程）
				callback.onAfter(null, request, response, null); // 请求结束回调
																	// （UI线程）
			}
		});
	}
	...
	/** 成功回调，失败结果 */
	public <T> void sendTokeninvalidCallback(final Request request,
			final Response response, final Exception e,
			final AbsCallback<T> callback) {
		OkHttpUtils.getInstance().getDelivery().post(new Runnable() {
			@Override
			public void run() {
				callback.onTokenInvalid(); // 请求失败回调 （UI线程）
				callback.onAfter(null, request, response, null); // 请求结束回调
																	// （UI线程）
			}
		});
	...
	/** 成功回调，发送到主线程 */
	public <T> void sendSuccessResultCallback(final T t, final Request request,
			final Response response, final AbsCallback<T> callback) {
		OkHttpUtils.getInstance().getDelivery().post(new Runnable() {
			@Override
			public void run() {
				callback.onSuccess(t); // 请求成功回调 （UI线程）
